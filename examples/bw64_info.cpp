...
int main(int argc, char const* argv[]) {
    ...
    std::cout << " - blockAlignment: " << bw64File->blockAlignment() << std::endl;
    std::cout << " - numberOfFrames: " << std::dec << bw64File->numberOfFrames() << ", 0x"
              << std::hex << bw64File->numberOfFrames() << std::endl;

    if (auto dataChunk = bw64File->dataChunk(); dataChunk) {
        std::cout << " - dataChunkSize: " << std::dec << dataChunk->size() << ", 0x" << std::hex
                  << dataChunk->size() << std::endl;
    }

    if (auto ds64Chunk = bw64File->ds64Chunk(); ds64Chunk) {
        std::cout << " - chunkSize64: " << std::dec << ds64Chunk->bw64Size() << ", 0x" << std::hex
                  << bw64File->ds64Chunk()->bw64Size() << std::endl;
    }
    std::cout << " - fileSize: " << std::dec << bw64File->fileSize() << ", 0x" << std::hex
              << bw64File->fileSize() << std::endl;
