...
TEST_CASE("encode_pcm_samples_8bit") {
    ...
    REQUIRE_THROWS_AS(utils::encodePcmSamples(samples, encoded8bit, 5, 8), std::runtime_error);
}
...
void checkDecodeEncodeAll() {
    ...
    for (uint64_t value = 0; value < (1 << bits); value++) checkDecodeEncodeOne<bytes, T>(value);
}
...
TEST_CASE("write_chunk_with_padding") {
    ...
    utils::writeChunk(stream, axmlChunk, static_cast<uint32_t>(axmlChunk->size()));
    unsigned int headerSize = 8u;
    unsigned int padding = 1u;
    REQUIRE(static_cast<uint64_t>(stream.tellp()) == headerSize + axmlChunk->size() + padding);
}
...
void checkLargerTypePositive() {
    ...
    CHECK_NOTHROW(utils::safeCast<T>(static_cast<U>((std::numeric_limits<T>::max)())));
    CHECK_THROWS_WITH(utils::safeCast<T>(static_cast<U>((std::numeric_limits<T>::max)()) + 1),
                      "overflow");
    CHECK_THROWS_WITH(utils::safeCast<T>((std::numeric_limits<U>::max)()), "overflow");
...
void checkLargerTypeNegative() {
    CHECK_NOTHROW(utils::safeCast<T>(static_cast<U>(0)));
    CHECK_NOTHROW(utils::safeCast<T>(static_cast<U>((std::numeric_limits<T>::min)())));
    CHECK_THROWS_WITH(utils::safeCast<T>(static_cast<U>((std::numeric_limits<T>::min)()) - 1),
                      "underflow");
    CHECK_THROWS_WITH(utils::safeCast<T>((std::numeric_limits<U>::min)()), "underflow");
...
void checkAddPositive() {
    ...
    CHECK_THROWS_WITH(utils::safeAdd<T>(1, (std::numeric_limits<T>::max)()), "overflow");
    CHECK_THROWS_WITH(utils::safeAdd<T>((std::numeric_limits<T>::max)(), 1), "overflow");

    CHECK_THROWS_WITH(
        utils::safeAdd<T>((std::numeric_limits<T>::max)(), (std::numeric_limits<T>::max)()),
        "overflow");
}
...
void checkAddNegative() {
    ...
    CHECK_THROWS_WITH(utils::safeAdd<T>(-1, (std::numeric_limits<T>::min)()), "underflow");
    CHECK_THROWS_WITH(utils::safeAdd<T>((std::numeric_limits<T>::min)(), -1), "underflow");

    CHECK_THROWS_WITH(
        utils::safeAdd<T>((std::numeric_limits<T>::min)(), (std::numeric_limits<T>::min)()),
        "underflow");
}
