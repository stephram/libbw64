...
TEST_CASE("format_info_chunk") {
    // basic test
    {
        ...
        auto formatInfoChunk = parseFormatInfoChunk(formatChunkStream, utils::fourCC("fmt "), 16);
    ...
    // wrong chunkSize
    {
        ...
        REQUIRE_THROWS_AS(parseFormatInfoChunk(formatChunkStream, utils::fourCC("fmt "), 20),
                          std::runtime_error);
    }
    // illegal formatTag
    {
        const char* formatChunkByteArray =
            ...
            "\x02\x00\x10\x00";  // blockAlignment = 2; bitsPerSample = 16
        std::istringstream formatChunkStream(std::string(formatChunkByteArray, 16));
        REQUIRE_THROWS_AS(parseFormatInfoChunk(formatChunkStream, utils::fourCC("fmt "), 16),
                          std::runtime_error);
    }
    // wrong channelCount
    {
    ...
    // read/write
    {
        ...
        auto formatChunkReread = parseFormatInfoChunk(stream, utils::fourCC("fmt "), 16);
    ...
    {
        REQUIRE_THROWS_WITH(FormatInfoChunk(0xffff, 48000, 24),
...
TEST_CASE("format_info_chunk_extradata") {
    SECTION("cbSize = 0") {
        ...
        auto formatInfoChunk = parseFormatInfoChunk(formatChunkStream, utils::fourCC("fmt "), 18);
        REQUIRE(formatInfoChunk->formatTag() == 1);
        REQUIRE(formatInfoChunk->channelCount() == 1);
        REQUIRE(formatInfoChunk->sampleRate() == 48000);
        REQUIRE(formatInfoChunk->bytesPerSecond() == 96000);
    ...
    SECTION("cbSize too large") {
        const char* formatChunkByteArray =
            ...
            "\x16\x00";  // cbSize = 22
        std::istringstream formatChunkStream(std::string(formatChunkByteArray, 18));
        REQUIRE_THROWS_AS(parseFormatInfoChunk(formatChunkStream, utils::fourCC("fmt "), 18),
                          std::runtime_error);
    }
    SECTION("extensible") {
        const char* formatChunkByteArray =
            ...
            "\x01\x00\x00\x00\x00\x00\x00\x10\x80\x00\x00\xaa\x00\x38\x9b"
            "\x71";  // KSDATAFORMAT_SUBTYPE_PCM
        std::istringstream formatChunkStream(std::string(formatChunkByteArray, 40));
        auto formatInfoChunk = parseFormatInfoChunk(formatChunkStream, utils::fourCC("fmt "), 40);
        ...
        REQUIRE(extraData->subFormatString() ==
                std::string("\x00\x00\x00\x00\x00\x10\x80\x00\x00\xaa\x00\x38\x9b\x71", 14));
    ...
    SECTION("PCM with extradata") {
        const char* formatChunkByteArray =
            ...
            "\x71";  // KSDATAFORMAT_SUBTYPE_PCM
        std::istringstream formatChunkStream(std::string(formatChunkByteArray, 40));
        REQUIRE_THROWS_AS(parseFormatInfoChunk(formatChunkStream, utils::fourCC("fmt "), 40),
                          std::runtime_error);
    }
}
...
TEST_CASE("chna_chunk") {
    ...
    // read/write
    {
        ...
        chnaChunk->addAudioId(AudioId(1, "ATU_00000001", "AT_00031001_01", "AP_00031001"));
        chnaChunk->addAudioId(AudioId(1, "ATU_00000002", "AT_00031002_01", "AP_00031002"));
        chnaChunk->addAudioId(AudioId(2, "ATU_00000003", "AT_00031003_01", "AP_00031003"));
    ...
    {  // wrong numTracks
        const char* chnaChunkByteArray =
            ...
            "\x00";  // padding
        std::istringstream chnaChunkStream(std::string(chnaChunkByteArray, 44));
        REQUIRE_THROWS_AS(parseChnaChunk(chnaChunkStream, utils::fourCC("chna"), 44),
                          std::runtime_error);
    }
    {  // wrong numUids
        const char* chnaChunkByteArray =
    ...
    {  // zero trackIndex throws when writing
        std::stringstream stream;
        auto chnaChunk = std::make_shared<ChnaChunk>();
        chnaChunk->addAudioId(AudioId(0, "ATU_00000001", "AT_00031001_01", "AP_00031001"));
...
TEST_CASE("ds64_chunk") {
    // basic test
    {
        ...
        auto ds64Chunk = parseDataSize64Chunk(ds64ChunkStream, utils::fourCC("ds64"), 40);
    ...
    {
        std::stringstream stream;
        auto dataSize64Chunk = std::make_shared<DataSize64Chunk>(987654321, 123456789);
        auto axmlId = utils::fourCC("axml");
        dataSize64Chunk->setChunkSize(axmlId, 654321);
        dataSize64Chunk->write(stream);
        auto dataSize64ChunkReread = parseDataSize64Chunk(stream, utils::fourCC("ds64"), 40);
    ...
    {  // wrong fourCC
        ...
        REQUIRE_THROWS_AS(parseDataSize64Chunk(ds64ChunkStream, utils::fourCC("ds65"), 8),
                          std::runtime_error);
    }
    {  // wrong size
        const char* ds64ChunkByteArray =
            "\x9a\xc6\x22\x31\xa5\x00\x00\x00";  // bw64Size = 709493966490
        std::istringstream ds64ChunkStream(std::string(ds64ChunkByteArray, 8));
        REQUIRE_THROWS_AS(parseDataSize64Chunk(ds64ChunkStream, utils::fourCC("ds64"), 8),
                          std::runtime_error);
    }
}
