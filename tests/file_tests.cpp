...
TEST_CASE("read_rect_24bit_wrong_fmt_size") {
    REQUIRE_THROWS_AS(Bw64Reader("rect_24bit_wrong_fmt_size.wav"), std::runtime_error);
}
...
void writeClipped(const std::string& filename, uint16_t bitDepth, uint64_t frames,
                  uint16_t channels = 1u, uint32_t sampleRate = 48000u) {
...
void writeRandom(const std::string& filename, uint16_t bitDepth, uint64_t frames,
                 uint16_t channels = 1u, uint32_t sampleRate = 48000u) {
...
TEST_CASE("write_read_big", "[.big]") {
    ...
    std::string filename = "big.wav";
    {
        ...
        for (size_t i = 0; i < block.size(); i++) block[i] = (i % 2 == 0) ? 0.5f : 0.0f;
...
TEST_CASE("write_too_many_big_chunks", "[.big]") {
    ...
    bw64File->setAxmlChunk(std::make_shared<MegaChunk>(utils::fourCC("mega"), 0x100000000ull));
    bw64File->setAxmlChunk(std::make_shared<MegaChunk>(utils::fourCC("megb"), 0x100000000ull));

    REQUIRE_THROWS_WITH(bw64File->close(),
                        "ds64 chunk is too large (52 bytes) to overwrite JUNK "
                        "chunk (40 bytes)");
