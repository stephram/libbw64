...
namespace bw64 {
    ...
    inline std::shared_ptr<ExtraData> parseExtraData(std::istream& stream) {
        ...
        return std::make_shared<ExtraData>(validBitsPerSample, dwChannelMask, subFormat,
                                           std::string(subFormatString, 14));
    }

    /// @brief Parse FormatInfoChunk from input stream
    inline std::shared_ptr<FormatInfoChunk> parseFormatInfoChunk(std::istream& stream, uint32_t id,
                                                                 uint64_t size) {
        ...
        if (formatTag == 1) {
            if (cbSize != 0) {
                throw std::runtime_error("WAVE_FORMAT_PCM fmt chunk should not have extra data");
            }
        ...
        if (formatInfoChunk->blockAlignment() != blockAlignment) {
            std::stringstream errorString;
            errorString << "sanity check failed. 'blockAlignment' is " << blockAlignment
                        << " but should be " << formatInfoChunk->blockAlignment();
            throw std::runtime_error(errorString.str());
        }
        if (formatInfoChunk->bytesPerSecond() != bytesPerSecond) {
            std::stringstream errorString;
            errorString << "sanity check failed. 'bytesPerSecond' is " << bytesPerSecond
                        << " but should be " << formatInfoChunk->bytesPerSecond();
            throw std::runtime_error(errorString.str());
        }
    ...
    inline std::shared_ptr<AxmlChunk> parseAxmlChunk(std::istream& stream, uint32_t id,
                                                     uint64_t size) {
        ...
        // since c++11, std::string[0] returns a valid reference to a null byte
        // for size==0
    ...
    inline std::shared_ptr<ChnaChunk> parseChnaChunk(std::istream& stream, uint32_t id,
                                                     uint64_t size) {
    ...
    inline std::shared_ptr<DataSize64Chunk> parseDataSize64Chunk(std::istream& stream, uint32_t id,
                                                                 uint64_t size) {
        ...
        if (!stream.good()) throw std::runtime_error("file error while seeking past ds64 chunk");

        return std::make_shared<DataSize64Chunk>(bw64Size, dataSize, table);
    }

    inline std::shared_ptr<DataChunk> parseDataChunk(std::istream& /* stream */, uint32_t id,
                                                     uint64_t size) {
    ...
    inline std::shared_ptr<Chunk> parseChunk(std::istream& stream, ChunkHeader header) {
        stream.clear();
        stream.seekg(header.position + 8u);
        if (!stream.good())
            throw std::runtime_error("file error while seeking past chunk header chunk");
