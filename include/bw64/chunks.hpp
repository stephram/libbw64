...
namespace bw64 {
    ...
    class UnknownChunk : public Chunk {
       public:
        ...
        void write(std::ostream& stream) const override {
            std::copy(data_.begin(), data_.end(), std::ostreambuf_iterator<char>(stream));
        }
    ...
    class ExtraData {
       public:
        ...
        ExtraData(uint16_t validBitsPerSample, uint32_t dwChannelMask, uint16_t subFormat,
    ...
    class FormatInfoChunk : public Chunk {
       public:
        ...
        FormatInfoChunk(uint16_t channels, uint32_t sampleRate, uint32_t bitDepth,
                        std::shared_ptr<ExtraData> extraData = nullptr, uint16_t formatTag = 1) {
            ...
            if (static_cast<int32_t>(channels) * bitDepth / 8 > 0xffff) {
                throw std::runtime_error(
                    "channelCount and bitsPerSample would overflow "
                    "blockAlignment");
            }
    ...
    class DataChunk : public Chunk {
       public:
         ...
         * override for this function is not used. Using this method will throw
         * an exception.
         */
        void write(std::ostream& /* stream */) const override {
            throw std::logic_error(
                "DataChunk::write method is not implemented. Use "
                "Bw64Writer::write "
                "instead.");
        }
    ...
    class AudioId {
       public:
        AudioId(uint16_t trackIndex, const std::string& uid, const std::string trackRef,
                const std::string& packRef) {
            if (uid.size() > 12) {
                std::stringstream errorString;
                errorString << "uid \'" << uid << "\' is too long (" << uid.size() << " > " << 12;
                throw std::runtime_error("uid is too long ");
            }
            if (trackRef.size() > 14) {
                std::stringstream errorString;
                errorString << "trackRef \'" << trackRef << "\' is too long (" << trackRef.size()
                            << " > " << 14;
                throw std::runtime_error("uid is too long ");
            }
            if (packRef.size() > 11) {
                std::stringstream errorString;
                errorString << "packRef \'" << packRef << "\' is too long (" << packRef.size()
                            << " > " << 11;
        ...
        void write(std::ostream& stream) const {
            if (trackIndex_ == 0)
                throw std::runtime_error("AudioId trackIndex is 1-based, so must not be zero");
    ...
    class ChnaChunk : public Chunk {
       public:
        ...
        uint16_t numUids() const { return utils::safeCast<uint16_t>(audioIds_.size()); }
    ...
    class DataSize64Chunk : public Chunk {
       public:
        ...
        DataSize64Chunk(uint64_t bw64Size = 0, uint64_t dataSize = 0,
        ...
        uint32_t tableLength() const { return utils::safeCast<uint32_t>(table_.size()); }
