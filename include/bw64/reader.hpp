...
namespace bw64 {
     ...
     * class, meaning that the file will be openend and initialized (parse
     * header, format etc.) on construction, and closed on destruction.
     */
    class Bw64Reader {
       public:
        /**
        ...
        Bw64Reader(const char* filename) {
            ...
            if (!dataChunk()) throw std::runtime_error("mandatory data chunk not found");
        ...
        /// destructor; this will close the file if it has not already been
        /// done, but it is recommended to call close() first to handle
        /// exceptions
        ~Bw64Reader() { close(); }
        ...
        uint64_t numberOfFrames() const { return dataChunk()->size() / blockAlignment(); }
        /// @brief Get block alignment
        uint16_t blockAlignment() const {
            return utils::safeCast<uint16_t>(static_cast<uint32_t>(channels()) * bitDepth() / 8);
        }
         ...
         *
         * @returns `std::shared_ptr` to DataSize64Chunk if present and
         * otherwise a nullptr.
         */
         ...
         * @brief Get 'fmt ' chunk
         *
         * @returns `std::shared_ptr` to FormatInfoChunk if present and
         * otherwise a nullptr.
         */
         ...
         *
         * @warning This method usually should not be called, as the acces to
         * the DataChunk is handled seperately by the Bw64Reader class .
         *
        ...
        void seek(int32_t offset, std::ios_base::seekdir way = std::ios::beg) {
            ...
            const int64_t dataStartPos = getChunkHeader(utils::fourCC("data")).position + 8;
            const int64_t framePos = dataStartPos + frame * static_cast<int64_t>(blockAlignment());

            fileStream_.seekg(framePos);

            if (!fileStream_.good()) throw std::runtime_error("file error while seeking");
        }
        ...
        uint64_t read(T* outBuffer, uint64_t frames) {
            ...
            if (frames) {
                ...
                if (fileStream_.eof()) throw std::runtime_error("file ended while reading frames");
                if (!fileStream_.good())
                    throw std::runtime_error("file error while reading frames");

                utils::decodePcmSamples(rawDataBuffer_.data(), outBuffer, frames * channels(),
                                        bitDepth());
            }
         ...
         */
        uint64_t tell() {
            return ((uint64_t)fileStream_.tellg() - getChunkHeader(utils::fourCC("data")).position -
                    8u) /
       ...
       private:
        void readRiffChunk() {
            ...
            if (fileFormat_ != utils::fourCC("RIFF") && fileFormat_ != utils::fourCC("BW64") &&
        ...
        void parseChunkHeaders() {
            ...
            while (fileStream_.tellg() + header_size <= end) {
                ...
                std::streamoff chunk_size = utils::safeCast<std::streamoff>(chunkHeader.size);
                if (chunk_size % 2 != 0) chunk_size = utils::safeAdd<std::streamoff>(chunk_size, 1);

                std::streamoff chunk_end =
                    utils::safeAdd<std::streamoff>(fileStream_.tellg(), chunk_size);

                if (chunk_end > end) throw std::runtime_error("chunk ends after end of file");
