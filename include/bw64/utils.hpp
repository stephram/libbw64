...
namespace bw64 {
    namespace utils {
        ...
        void readValue(std::istream& stream, T& dest) {
            ...
            if (stream.eof()) throw std::runtime_error("file ended while reading value");
            if (!stream.good()) throw std::runtime_error("file error while reading value");
        }
        ...
        inline void readChunk(std::istream& stream, char* dest, size_t size) {
            if (size) {
                ...
                if (stream.eof()) throw std::runtime_error("file ended while reading chunk");
                if (!stream.good()) throw std::runtime_error("file error while reading chunk");
            }
        }
        ...
        template <typename T,
                  typename std::enable_if<
                      std::is_integral<typename std::remove_extent<T>::type>::value, int>::type = 0>
        ...
        inline void writeChunkPlaceholder(std::ostream& stream, uint32_t id, uint32_t size) {
        ...
        void encode(T value, char* buffer) {
            ...
            constexpr IntT maxval = static_cast<IntT>((UnsignedT{1} << (bits - 1)) - 1);
            ...
            for (size_t i = 0; i < bytes; i++) buffer[i] = (value_int >> (8 * i)) & 0xff;
        }
        ...
        T decode(const char* buffer) {
            ...
            // when converting from a char to an int, sign extension occurs and
            // the high bit is replicated to the high bytes
            //
            // we don't want this for the low bytes (since the high bit of these
            // is not a sign bit), but we do for the high byte. for example when
            // reading 24 bits into an int32, the high byte needs to be filled
            // with the sign bit
            //
            // an explicit cast to IntT is used because int can technically be
            // 16 bits
            IntT value = 0;
        ...
        template <typename T,
                  typename std::enable_if<std::is_floating_point<T>::value, int>::type = 0>
        void decodePcmSamples(const char* inBuffer, T* outBuffer, uint64_t numberOfSamples,
        ...
        void encodePcmSamples(const T* inBuffer, char* outBuffer, uint64_t numberOfSamples,
                              uint16_t bitsPerSample) {
            if (bitsPerSample == 16) {
                for (uint64_t i = 0; i < numberOfSamples; ++i) {
                ...
                for (uint64_t i = 0; i < numberOfSamples; ++i) {
                    ...
                    encode<4, int32_t>(static_cast<double>(inBuffer[i]), outBuffer + 4 * i);
                }
            } else {
                std::stringstream errorString;
                errorString << "unsupported number of bits: " << bitsPerSample;
        ...
        struct NeedToCheckLower {
            ...
            // convert limits to signed, otherwise this comparison can be
            // promoted to unsigned
            static constexpr bool value = static_cast<FromS>((std::numeric_limits<From>::min)()) <
                                          static_cast<ToS>((std::numeric_limits<To>::min)());
        };

        /// check x against the minimum value that To can hold
        template <typename To, typename From>
        typename std::enable_if<NeedToCheckLower<To, From>::value>::type checkLower(From x) {
            using FromS = typename std::make_signed<From>::type;

            // - From is signed
            // - To may be unsigned
            //   - if it's unsigned, this is ok because the lower limit of to is
            //   0
        ...
        template <typename To, typename From>
        typename std::enable_if<!NeedToCheckLower<To, From>::value>::type checkLower(From) {}

        /// convert signed or unsigned integer x to To, checking for overflow
        /// and underflow
        template <typename To, typename From>
        To safeCast(From x) {
            checkUpper<To>(x);
        ...
        /// add x and y, checking for overflow and underflow. X and Y most both
        /// be representable by T; use safeCast to make sure
        template <typename T>
        T safeAdd(T x, T y) {
